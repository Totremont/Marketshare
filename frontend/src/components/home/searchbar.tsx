...
export default function SearchBar(props : {setTextWritten : any})
{
    ...
    const view = 
    <div onClick={onClick} className={`border min-w-[200px] max-w-[300px] rounded-md border-slate-600 p-2 my-4 ${isFocused ? 'border-slate-400' : ''}`}>
    <div className="flex items-center static">
        {isWriting ? <button className={`text-sm py-1 px-1 rounded-md bg-[${Colors.OLIVE}]`}>Buscar</button> :
